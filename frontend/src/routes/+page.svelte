...
<div class="min-h-screen w-full flex flex-col lg:flex-row bg-[#FFFFFF] dark:bg-[#090D16] text-slate-900 dark:text-slate-100 font-sans overflow-hidden relative">
	...
	<div class="absolute -top-32 -left-32 w-[500px] h-[500px] bg-sky-200/40 dark:bg-sky-950/30 rounded-full blur-[120px] pointer-events-none animate-cloud-float"></div>
	<div class="absolute -bottom-32 -right-32 w-[500px] h-[500px] bg-sky-100/60 dark:bg-sky-900/20 rounded-full blur-[140px] pointer-events-none animate-cloud-float" style="animation-delay: 4s;"></div>
	...
	<div class="hidden lg:flex w-1/2 min-h-screen flex-col justify-between p-12 lg:p-16 border-r border-sky-100 dark:border-slate-800 relative bg-gradient-to-br from-white via-sky-50/40 to-sky-100/30 dark:from-slate-900 dark:via-slate-900 dark:to-slate-950">
		...
		<div class="relative z-10 flex items-center gap-3">
			...
			<div>
				<h1 class="text-xl font-bold tracking-tight text-slate-900 dark:text-white">DeepPhotos</h1>
				<p class="text-xs text-slate-500 dark:text-slate-400">Minimal Sky & Cloud Photo Vault</p>
			</div>
		</div>

		<!-- Center: Photo Cards Display -->
		<div class="relative z-10 my-auto py-6">
			<div class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-sky-100/80 dark:bg-sky-950 border border-sky-200 dark:border-sky-800 text-sky-800 dark:text-sky-300 text-xs font-medium mb-4">
				<Cloud class="w-3.5 h-3.5 text-sky-500" />
				<span>Clean & Minimal Experience</span>
			</div>

			<h2 class="text-3xl font-extrabold tracking-tight text-slate-900 dark:text-white mb-2">
				Store your memories under <br/>
				<span class="text-sky-500">clear blue skies.</span>
			</h2>
			<p class="text-sm text-slate-600 dark:text-slate-300 max-w-sm mb-8">
				...
				{#each photoCards as card}
					<div class="p-3.5 rounded-2xl cloud-card flex items-center justify-between transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5">
						<div class="flex items-center gap-3">
							<div class={`w-11 h-11 rounded-xl bg-gradient-to-br ${card.bg} dark:from-slate-800 dark:to-slate-900 border border-sky-200/70 dark:border-slate-700 flex items-center justify-center text-sky-600 dark:text-sky-400`}>
								<ImageIcon class="w-5 h-5" />
							</div>
							<div>
								<p class="text-xs font-bold text-slate-800 dark:text-slate-100">{card.title}</p>
								<p class="text-[11px] text-slate-500 dark:text-slate-400">{card.date}</p>
							</div>
						</div>
						<span class="text-[11px] text-sky-700 dark:text-sky-300 px-2.5 py-0.5 rounded-full bg-sky-100 dark:bg-sky-950 border border-sky-200 dark:border-sky-800 font-medium">
							{card.tag}
						</span>
					</div>
				{/each}
	...
	<div class="w-full lg:w-1/2 min-h-screen flex items-center justify-center p-6 md:p-12 relative z-10">
		...
		<div class="w-full max-w-sm animate-fade-in">
			...
			<div class="flex lg:hidden flex-col items-center mb-8 text-center">
				...
				<h1 class="text-xl font-bold text-slate-900 dark:text-white">DeepPhotos</h1>
				<p class="text-xs text-slate-500 dark:text-slate-400">Minimal Photo Vault</p>
			</div>
			...
			<div class="p-8 rounded-3xl cloud-card">
				...
				<div class="mb-6">
					<h3 class="text-xl font-bold text-slate-900 dark:text-white">Sign In</h3>
					<p class="text-xs text-slate-500 dark:text-slate-400 mt-1">Enter your details to access your photos</p>
				</div>
				...
				<form onsubmit={handleLogin} class="space-y-4">
					...
					<div class="space-y-1.5">
						<label for="username" class="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
					...
					<!-- Password Field -->
					<div class="space-y-1.5">
						<label for="password" class="text-xs font-semibold text-slate-700 dark:text-slate-300 flex items-center gap-1.5">
			...
			<p class="block lg:hidden text-center text-xs text-slate-500 dark:text-slate-400 mt-6">
				© {new Date().getFullYear()} DeepPhotos. All rights reserved.
			</p>
